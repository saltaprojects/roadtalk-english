...
export const ListeningExerciseViewer = ({
  exercise,
  onClose
  ...
  return <div className="min-h-screen bg-muted/30">
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="relative w-full h-64 rounded-lg overflow-hidden bg-cover bg-center" style={{
        ...
        {(isPlaying || hasPlayedOnce) && currentDialogueLine && <Card className="p-6 bg-card/95 backdrop-blur">
            <div className="flex items-start gap-4 mb-4">
              <Avatar className="w-12 h-12">
                {exercise.characterImage && <AvatarImage src={exercise.characterImage} />}
                <AvatarFallback>
        ...
        <Card className="p-6">
          <div className="flex flex-col items-center gap-4">
            <div className="flex items-center gap-4">
              ...
              {hasPlayedOnce && !isPlaying && <Button size="lg" onClick={handleReplay} variant="outline" className="gap-2">
        ...
        {showQuestion && <Card className="p-6">
            ...
            <div className="space-y-3 mb-6">
              {options.map((option, index) => <button key={index} onClick={() => !hasSubmitted && setSelectedAnswer(option.text)} disabled={hasSubmitted} className={`w-full p-4 text-left rounded-lg border-2 transition-all ${selectedAnswer === option.text ? hasSubmitted ? option.isCorrect ? 'border-green-500 bg-green-50 dark:bg-green-950' : 'border-red-500 bg-red-50 dark:bg-red-950' : 'border-primary bg-primary/5' : hasSubmitted && option.isCorrect ? 'border-green-500 bg-green-50 dark:bg-green-950' : 'border-muted hover:border-primary/50'}`}>
                  {option.text}
                </button>)}
            </div>

            {!hasSubmitted && selectedAnswer && <Button onClick={handleSubmit} size="lg" className="w-full">
                {t('listeningPractice.submit')}
              </Button>}

            {hasSubmitted && <div className="space-y-4">
                <div className={`p-4 rounded-lg ${isCorrect ? 'bg-green-100 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
                  <p className="font-bold text-lg">
                    {isCorrect ? t('listeningPractice.correct') : t('listeningPractice.incorrect')}
                  </p>
                  {!isCorrect && <p className="mt-2">
